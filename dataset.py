...
class CustomDataset(Dataset):
    def __init__(
        ...
        with open(self.folder + "/transforms.json", "r") as json_file:
            self.camera_json = json.load(json_file)['frames']
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img = self.transform(img)
        mat = torch.FloatTensor(self.camera_json[index]['transform_matrix'])
        mat = torch.cat((mat[:3,:3].flatten(),mat[:3,3]))
        return {'img': img, 'index': index, 'camera': mat}